pipeline {
    ...
    environment {
        FRONTEND_IMAGE = "sachinoct/frontend-app:latest"  // Update with your DockerHub username
        BACKEND_IMAGE = "sachinoct/backend-app:latest"
        FRONTEND_CONTAINER = "frontend-container"
        BACKEND_CONTAINER = "backend-container"
        REGISTRY_CREDENTIALS = "docker-sachin"  // Jenkins credentials ID for Docker login
    }

    stages {
        stage('Checkout Code') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'git_SachinOct07', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_TOKEN')]) {
                    git url: "https://$GIT_USER:$GIT_TOKEN@github.com/SachinOct07/Terraform.git", branch: 'main'
                }
            }
        }
